...
function findNearestVariableName(path2, directive) {
...
function findDirectives(ast, opts) {
  ...
  if (hasFileDirective) {
    babel__namespace.traverse(ast, {
      ExportDefaultDeclaration(path2) {
        if (babel__namespace.types.isFunctionDeclaration(path2.node.declaration)) {
          compileDirective(
      ...
      ExportDeclaration(path2) {
        if (babel__namespace.types.isExportNamedDeclaration(path2.node) && babel__namespace.types.isVariableDeclaration(path2.node.declaration) && (babel__namespace.types.isFunctionExpression(
  ...
  function compileDirective(directiveFn, compileDirectiveOpts2) {
    ...
    if ((babel__namespace.types.isExportNamedDeclaration(directiveFn.parentPath.node) || compileDirectiveOpts2.isDirectiveSplitParam && babel__namespace.types.isExportDefaultDeclaration(
      directiveFn.parentPath.node
    )) && (babel__namespace.types.isFunctionDeclaration(directiveFn.node) || babel__namespace.types.isFunctionExpression(directiveFn.node)) && babel__namespace.types.isIdentifier(directiveFn.node.id)) {
      const originalFunctionName = directiveFn.node.id.name;
      programBody.splice(
        topParentIndex + 1,
        0,
...
const safeRemoveExports = (ast) => {
  ast.program.body = ast.program.body.flatMap((node) => {
    if (babel__namespace.types.isExportNamedDeclaration(node) || babel__namespace.types.isExportDefaultDeclaration(node)) {
      if (babel__namespace.types.isFunctionDeclaration(node.declaration) || babel__namespace.types.isClassDeclaration(node.declaration) || babel__namespace.types.isVariableDeclaration(node.declaration)) {
        if (babel__namespace.types.isFunctionDeclaration(node.declaration) || babel__namespace.types.isClassDeclaration(node.declaration)) {
